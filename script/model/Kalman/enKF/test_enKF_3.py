...
- Train rééchantillonné journalier
- Test: observations laissées inchangées (sparse)
...
Ridge_alpha = 1.0
SEED = 42
OUTPUT_PDF = "../results/prediction/enKF/v3/predictions_report_Q=" + str(Q_scale) + "_" + str(DEPTH_TARGET) + "m.pdf"
...
# Nouveau : estimation pondérée de A
def estimate_A_weighted(Xs, Xn, var_cols, feature_weights=None, target_weights=None, alpha=1.0, fit_intercept=True):
    """
    Estime A et b avec pondération des features (entrées) et targets (sorties).
    feature_weights: dict var->float appliqué aux colonnes de Xs
    target_weights: dict var->float appliqué aux colonnes de Xn
    var_cols: liste ordonnée des noms de colonnes correspondantes aux colonnes de Xs/Xn
    Retourne: A (n_vars x n_vars), b (n_vars,), Q (n_vars x n_vars), resid
    """
    ...
    # éviter zéros (sinon division par zéro)
    s[s == 0.0] = 1e-8
    t[t == 0.0] = 1e-8

    # scale columns (col-wise multiplication)
    Xs_scaled = Xs * s.reshape(1, -1)
    Xn_scaled = Xn * t.reshape(1, -1)

    # estimer A_scaled, b_scaled
    ...
    # Repasser à l'échelle originale:
    # calcul matriciel sûr (S = diag(s), T = diag(t))
    S = np.diag(s)
    T = np.diag(t)
    # On a: Xn_scaled = Xs_scaled @ A_scaled.T + b_scaled
    # => Xn = Xs @ (S @ (A_scaled.T @ T^{-1})) + b_scaled @ T^{-1}
    # Donc A_original.T = S @ (A_scaled.T @ T^{-1})
    # et b_original = b_scaled / t (élément par élément)
    try:
        T_inv = np.linalg.inv(T)
    except np.linalg.LinAlgError:
        # T diagonal non-inversible improbable car on a évité zéros, mais fallback
        T_inv = np.diag(1.0 / np.where(np.diag(T) == 0.0, 1e-8, np.diag(T)))
    A_original_T = S.dot(A_scaled.T.dot(T_inv))
    A = A_original_T.T
    b = b_scaled / np.diag(T)

    # residus & Q en échelle originale
...
def main():
    ...
    # (on supprime les colonnes correspondantes si poids == 0)
    to_remove = []
    ...
    if len(to_remove) > 0:
        ...
        # drop des colonnes dans train/test si présentes
    ...
    # sauvegarde résidus pour analyse locale
    import matplotlib.pyplot as _plt
    _fig = _plt.figure(figsize=(6, 3))
    _plt.hist(resid.flatten(), bins=100)
    _plt.title("Histogramme résidus (t+1 - pred)")
    _plt.tight_layout()
    _plt.show()
    ...
    # compute metrics using ORIGINAL sparse observations (test_original)
    metrics = {}
    for v in var_cols:
