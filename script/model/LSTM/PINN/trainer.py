...
class Trainer:
    def __init__(self, model, config, pinns=None):
        ...
        # Optimizer includes model and PINN parameters
    ...
    def train(self, train_loader, val_loader, feature_cols, target_names, scaler_y):
        ...
        # Component weights for weighted MSE
        ...
        for epoch in range(1, self.config.N_EPOCHS + 1):
            ...
            for xb, yb in train_loader:
                ...
                for pinn in self.pinns:
                    # Apply individual pinn weight (handled inside pinn usually, but we have global config too)
                    l = pinn.compute_pinn_loss(xb, out, meta)
                    if l is not None:
                        # You might want to apply specific weights from config here if not in PINN class
                        # For now assume PINN class handles specific weights, and we apply global PINN weight
            ...
            if patience_cnt >= self.config.PATIENCE:
                print(f"Early stopping triggered. Best val loss={best_val:.6f}.")
                break
