# -*- coding: utf-8 -*-
"""
Version : même pipeline, mais boucle sur toutes les combinaisons de targets et
génère un PDF par combinaison.
"""
import os
# --- MUST set deterministic env-vars before importing TensorFlow ---
...
OUTPUT_PDF_TEMPLATE = f"../../../results/prediction/LSTM/v4/{AGG_METHOD}/LSTM_predictions_bis_{AGG_METHOD}__{{names}}.pdf"
...
# --- liste complète des targets possibles (décommenter/ajouter si nécessaire) ---
ALL_TARGETS = [
    "temperature (°C)",
    "chlorophyll (mg m-3)",
    "turbidity (NTU)",
...
def build_lstm(n_timesteps, n_features, n_outputs, lr=1e-3, dropout=0.2, component_weights=None):
    model = Sequential()
    ...
    if component_weights is None:
        w = np.ones(n_outputs, dtype=float)
    else:
...
def predict_walk_forward(model, scaler_X, scaler_y, daily_full_tf, feature_cols, target_count, seq_len, train_end_dt, test_df_tf, batch_size):
...
def predict_recursive(model, scaler_X, scaler_y, history_orig, feature_cols, target_count, seq_len, future_dates, batch_size):
    ...
    for t in range(n_steps):
        ...
        p_scaled = model.predict(cur_scaled, verbose=0, batch_size=batch_size)[0]
...
# ---------------------- Pipeline réutilisable pour une combinaison donnée ----------------------
def run_pipeline(target_cols, output_pdf_template):
    ...
    model = build_lstm(SEQUENCE_LENGTH, n_features, n_outputs, lr=LEARNING_RATE, component_weights=component_weights)
    ...
    if RECURSIVE_FORECAST:
        ...
        preds_targets = predict_recursive(model, scaler_X, scaler_y, history_orig, feature_cols, len(target_cols), SEQUENCE_LENGTH, future_dates, batch_size=1)
        true_targets = test_df_tf[target_cols].values[:len(preds_targets)]
        dates = future_dates
    else:
        preds_targets, true_targets, dates = predict_walk_forward(model, scaler_X, scaler_y, daily_full_tf, feature_cols, len(target_cols), SEQUENCE_LENGTH, train_end_dt, test_df_tf, batch_size=BATCH_SIZE)
    ...
    # safe join des noms (évite caractères spéciaux)
...
# ---------------------- Main: boucle sur toutes les combinaisons ----------------------
def main():
    # choisir ici si on veut toutes les combinaisons ou un sous-ensemble
    all_targets = ALL_TARGETS.copy()
    n = len(all_targets)
    combos = []
    for r in range(1, n+1):
        combos.extend(itertools.combinations(all_targets, r))

    print(f"Nombre de combinaisons à exécuter: {len(combos)} (1..{n})")

    for combo in combos:
        target_cols = list(combo)
        try:
            run_pipeline(target_cols, OUTPUT_PDF_TEMPLATE)
        except Exception as e:
            print(f"Erreur pour la combinaison {target_cols}: {e}")
            traceback.print_exc()
            # on continue sur la prochaine combinaison
