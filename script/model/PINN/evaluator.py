...
class Evaluator:
    """
    Évalue le modèle sur la période de test et génère le rapport PDF.
    Applique une correction OLS (moindres carrés ordinaires) sur les prédictions
    pour absorber les biais systématiques avant le calcul des métriques.
    """
    ...
    def evaluate(self):
        ...
        # Correction OLS et calcul des métriques par cible
        best_params = {}
        ...
        for i, col in enumerate(target_cols):
            ...
            # Régression linéaire OLS : t ≈ bx * p + by
    ...
    def _predict_recursive(self):
        """
        Prévision récursive : à chaque pas, la prédiction est réinjectée comme entrée.
        Simule un scénario réel où les observations futures ne sont pas disponibles.
        """
        ...
        if hist_start_idx < 0:
            raise ValueError("Pas assez d'historique pour le forecast récursif.")
        ...
        with torch.no_grad():
            for t in range(n_steps):
                ...
                # Décalage de la fenêtre : les cibles prédites remplacent les observations
                row = np.array(current[-1, :], copy=True)
                row[:target_count] = p_inv

                # Mise à jour des caractéristiques temporelles (doy)
    ...
    def _predict_walk_forward(self):
        """
        Prévision walk-forward : à chaque pas, les vraies valeurs passées sont utilisées.
        Donne une borne supérieure des performances du modèle.
        """
        daily_full_tf = self.dm.daily_full_tf
        feature_cols = self.dm.feature_cols
        target_cols = self.dm.target_cols
